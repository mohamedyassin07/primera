...
if ( $nm_theme_options['product_share_buttons'] ) {
    ...
    $share_links = apply_filters( 'nm_product_share_links', array(
        ...
        '<a href="//wa.me/?text=' . urlencode( get_the_title() ) . '%20' . $esc_permalink . '" target="_blank" title="' . esc_html__( 'WhatsApp', 'nm-framework' ) . '"><i class="nm-font nm-font-whatsapp"></i></a>',
        '<a href="//telegram.me/share/url?url=' . $esc_permalink . '&amp;text=' . urlencode( get_the_title() ) .'" target="_blank" title="' . esc_html__( 'Telegram', 'nm-framework' ) . '"><i class="nm-font nm-font-telegram"></i></a>'
    ) );
} else {
    $wrapper_class = '';
}
